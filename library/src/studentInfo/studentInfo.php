   ...
   //    $sqlQuery = "SELECT * FROM `student_promotion` where yr='$yr'";  
    ...
    public function indexCurrentYear(){
        if($this->grpId=="0"){
            ...
            where c_id ='$this->cId' and section_id='$this->secId' order by roll";  
        }
       else{ 
            $sqlQuery = "SELECT * from v_current_year_student 
    ...
    public function indexAttendance(){
       // $sqlQuery = "SELECT * from gpa 
        //where c_id ='$this->cId' and section_id='$this->secId' and grp_id='$this->grpId' order by roll"; 
        
        $sqlQuery = "SELECT * FROM `v_current_year_student` where c_id ='$this->cId' and section_id='$this->secId' and grp_id='$this->grpId' order by roll";
        
        /* $sqlQuery = "SELECT g.*,s.name,p.roll from total_working_day g
                    left join student_promotion p on (p.s_id=g.s_id and p.c_id=g.c_id and p.yr=g.yr) 
                    LEFT JOIN student_info s ON s.id=g.s_id
                    where g.c_id ='$this->cId' and  p.section_id='$this->secId' and g.term_id='$this->tId' and p.grp_id='$this->grpId' order by p.roll";
         */
    ...
    public function clsWiseStudentList(){
        ...
        return $allData;
    }//end of clsWiseStudentList()
    
    public function stdTrackList(){
        $yr = date('Y'); 
        if($_GET['searchKey']=='') $searchKey='No Information';
        else $searchKey = $_GET['searchKey'];
        $sqlQuery = "SELECT a.*,c.c_name,m.tuition_fee,g.group_name,s.sec_name,(m.tuition_fee-a.discount_fee) AS net_tuition_fee
                    FROM (
                        SELECT p.id,f.id AS s_id,f.name,f.f_name,f.f_ocp,f.m_name,f.m_ocp,f.gender,f.religion,f.gur_name,f.gur_mobile,
                        f.std_mobile,f.std_email,f.dob,f.pr_vill,f.pr_po,f.pr_ps,f.pr_dist,f.pm_vill,f.pm_po,f.pm_ps,f.pm_dist,f.blood_group,p.created_at,p.yr,p.amount,p.discount_fee,p.c_id,p.roll,p.grp_id,p.section_id,p.card_no FROM `student_info` f
                        LEFT JOIN `student_promotion` p ON p.s_id=f.id
                        WHERE (p.is_trashed='No' AND f.is_trashed='No')
                    ) a
                    LEFT JOIN `class` c ON c.c_id=a.c_id
                    ...
                    LEFT JOIN `grp` g ON g.id=a.grp_id
                    LEFT JOIN `section` s ON s.id=a.section_id
                    WHERE a.name like '%$searchKey%' or  a.m_name like '%$searchKey%' or  a.gur_name like '%$searchKey%' or  a.gur_mobile like '%$searchKey%' or  a.blood_group like '%$searchKey%'
                    or  a.s_id like '%$searchKey%' or  a.std_email like '%$searchKey%'  or  a.roll like '%$searchKey%'  or  a.c_id like '%$searchKey%'  or  c.c_name like '%$searchKey%'
                    ORDER BY a.grp_id,a.section_id,a.roll";
    ...
    public function clsWiseStudentListSep(){
        
        $sqlQuery = "SELECT a.*,c.c_name,m.tuition_fee,g.group_name,s.sec_name,(m.tuition_fee-a.discount_fee) AS net_tuition_fee
                    FROM (
                        SELECT p.id,f.id AS s_id,f.name,f.f_name,f.f_ocp,f.m_name,f.m_ocp,f.gender,f.religion,f.gur_name,f.gur_mobile,f.std_mobile,f.std_email,f.dob,f.pr_vill,f.pr_po,f.pr_ps,f.pr_dist,f.pm_vill,f.pm_po,f.pm_ps,f.pm_dist,f.blood_group,p.created_at,p.yr,p.amount,p.discount_fee,p.c_id,p.roll,p.grp_id,p.section_id,p.card_no FROM `student_info` f
                        LEFT JOIN `student_promotion` p ON p.s_id=f.id
                        WHERE (p.is_trashed='No' AND f.is_trashed='No') AND
                        (p.yr=$this->yr AND p.c_id =$this->cId AND p.grp_id =$this->grpId AND p.section_id =$this->secId)
                    ) a
                    LEFT JOIN `class` c ON c.c_id=a.c_id
                    LEFT JOIN `class_payment` m ON (m.c_id=a.c_id AND m.grp_id=a.grp_id)
                    LEFT JOIN `grp` g ON g.id=a.grp_id
        ...
        return $allData;
    }//end of clsWiseStudentListSep()
    
    public function clsWiseStudentListCda(){
        $yr = date('Y'); 
        $sqlQuery = "SELECT a.*,c.c_name,m.tuition_fee,g.group_name,s.sec_name,(m.tuition_fee-a.discount_fee) AS net_tuition_fee
                    FROM (
                        SELECT p.id,f.id AS s_id,f.name,f.f_name,f.f_ocp,f.m_name,f.m_ocp,f.gender,f.religion,f.gur_name,f.gur_mobile,f.std_mobile,f.std_email,f.dob,f.pr_vill,f.pr_po,f.pr_ps,f.pr_dist,f.pm_vill,f.pm_po,f.pm_ps,f.pm_dist,f.blood_group,p.created_at,p.yr,p.amount,p.discount_fee,p.c_id,p.roll,p.grp_id,p.section_id,p.card_no,f.house,f.sn_no FROM `student_info` f
                        LEFT JOIN `student_promotion` p ON p.s_id=f.id
                        WHERE (p.is_trashed='No' AND f.is_trashed='No')
                    ) a
                    LEFT JOIN `class` c ON c.c_id=a.c_id
                    ...
                    LEFT JOIN `grp` g ON g.id=a.grp_id
                    LEFT JOIN `section` s ON s.id=a.section_id
                    WHERE a.yr=$yr AND a.c_id =$this->cId AND g.id=$this->grpId AND s.id=$this->secId
                    /* AND a.roll in (12) */
                    ORDER BY a.grp_id,a.section_id,a.roll";
    ...
    public function singleStudentClasswise(){
        $yr = date('Y');
               $sqlQuery = "SELECT a.*,c.c_name,m.tuition_fee,g.group_name,s.sec_name,(m.tuition_fee-a.discount_fee) AS net_tuition_fee
                    ...
                    // WHERE a.roll = $this->roll AND c.c_id= $this->cId AND a.section_id= $this->secId AND a.grp_id= $this->grpId";
        $STH = $this->DBH->query($sqlQuery);
