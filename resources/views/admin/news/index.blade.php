...
@section('content')
    ...
    <div class="card">
            ...
            <table class="table table-vcenter">
                ...
                <tbody>
                    @foreach($newsList as $item)
                        <tr>
                            <td><a href="{{ route('news.show', $item) }}" target="_blank" class="text-reset">{{ Str::limit($item->title, 60) }}</a></td>
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">
                                    <a href="{{ route('admin.news.edit', $item) }}" class="btn btn-outline-warning">Edit</a>
        ...
        @if(method_exists($newsList, 'links'))
            <div class="card-footer">{{ $newsList->links("pagination::bootstrap-5") }}</div>
        @endif
    </div>
@endsection
