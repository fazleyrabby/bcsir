	...
	<div class="widget red">
     <div class="widget-title">
       <?php if(isset($_GET['expense'])) { 
         if ($_GET['expense'] == "add_expense"){
           echo "<h4><i class='icon-tags'></i> Add Expense </h4>";
         }
         elseif($_GET['expense'] == "expense_list")
         {
            echo "<h4><i class='icon-tags'></i> Expense List </h4>";
         }
        }
        ?>
     	</div>
     ...
     <div class="span12">
		...
		<?php
		if (isset($_GET['expense'])) {    //if add expense found
			$expense = $_GET['expense'];     //if clicked on add expense
			if ($expense == "add_expense") { ?>
			<form action="Action/add_expense.php" method="POST" class="form-horizontal" enctype=multipart/form-data>
            <input type="hidden" value="<?php echo $usernamee?>" name="username">
                ...
                <div class="control-group">
                    <label class="control-label">খরচ টাইটেল</label>
                        <div class="controls">
                            <input type="text" class="span6 tooltips" data-trigger="hover" data-original-title="Type account head title"  name="expense_title" required/>
                        </div>
                </div>
                <div class="control-group">
                    <label class="control-label">টোটাল খরচ</label> <!--total expense-->
                        <div class="controls">
                            <input type="number" class="span6 tooltips" data-trigger="hover" data-original-title="Type account head title"  name="total_expense" required/>
                        </div>
                </div>
                <div class="control-group">
                    <label class="control-label">পেমেন্ট</label> <!--paid expense-->
                        <div class="controls">
                ...
                <div class="control-group">
                    <label class="control-label">খরচ বকেয়া</label>  <!-- due expense -->
                        <div class="controls">
                            <input type="number" class="span6 tooltips" data-trigger="hover" data-original-title="Type account head title"  name="due_expense" required>
                        </div>
                </div>
                <div class="control-group">
				...
				<div class="control-group">
					<div class="controls">
						<input type="submit" name="add_expense" value="সাবমিট ">
					</div>
				</div>
			...
			<?php }
			elseif ($expense == "expense_list") { ?> <!--  if clicked on account head List  -->
               ...
               <?php  
                 ...
                 if($list_expense==true)
                 {
                    while($data=mysqli_fetch_array($list_expense))
                        ...
                        <td><a href='add_expense.php?expense=update_expense&expense_id=$expense_id'><span class='btn btn-success btn_success_custom'>সংশোধন </span></a>
                        &nbsp;&nbsp;&nbsp;&nbsp;
                        <a href='Action/add_expense.php?expense_id=$expense_id&expense_delete=expense_delete'>
                        <span class='btn btn-danger'>মুছে ফেলুন</span>
                        </a>&nbsp;&nbsp;&nbsp;&nbsp;
                        <a href='add_expense.php?expense_id=$expense_id&expense=expense_details'>
        ...
        <?php }
			...
			elseif ($expense == "update_expense") {

			  if (isset($_GET['expense_id'])) {
			  	 $expense_id = $_GET['expense_id'];
                 $edit_expense=mysqli_query($conn,"SELECT * FROM manage_expense WHERE expense_id='$expense_id'");  
                 if($edit_expense==true)
                 {
                  while($data=mysqli_fetch_array($edit_expense))
                    {
                        //$expense_id=$data['expense_id'];
                        $expense_serial=$data['serial_no'];
                        $account_head_expense=$data['account_head_id'];
 	        ...
 	        <form action="Action/add_expense.php" method="POST" class="form-horizontal" enctype=multipart/form-data>
                <input type="hidden" value="<?php echo $usernamee?>" name="username">
                <input type="hidden" value="<?php echo $expense_id?>" name="expense_id">
                ...
                <div class="control-group">
                    ...
                    <select name="account_head" class="span6  tooltips" data-original-title="Show on Menu">
                    <?php 
                        $account_head = mysqli_query($conn,"SELECT account_head_id,account_head_name FROM account_head 
                        WHERE account_head_st = 1"); //1 = only active 
                        if($account_head == true) {
                          while($data = mysqli_fetch_array($account_head)){
                            print_r($data);
                            $account_head_id = $data['0'];
                ...
                <div class="control-group">
                    <label class="control-label">সিরিয়াল</label>
                        <div class="controls">
                            <input type="text" class="span6 tooltips" data-trigger="hover" data-original-title="Type account head title"  name="serial_no" required value="<?=$expense_serial?>" disabled/>
                            </div>
                    </div>
                    <div class="control-group">
                        <label class="control-label">খরচ টাইটেল</label>
                            <div class="controls">
                                <input type="text" class="span6 tooltips" data-trigger="hover" data-original-title="Type account head title"  name="expense_title" required value="<?=$expense_title?>"/>
                            </div>
                    </div>
                    <div class="control-group">
                        <label class="control-label">টোটাল খরচ</label> <!--total expense-->
                            <div class="controls">
                                <input type="number" class="span6 tooltips" data-trigger="hover" data-original-title="Type account head title"  name="total_expense" required value="<?=$total_amount?>" />
                            </div>
                    </div>
                    <div class="control-group">
                        <label class="control-label">পেমেন্ট</label> <!--paid expense-->
                    ...
                    <div class="control-group">
                        <label class="control-label">খরচ বকেয়া</label>  <!-- due expense -->
                            <div class="controls">
                                <input type="number" class="span6 tooltips" data-trigger="hover" data-original-title="Type account head title"  name="due_expense" required value="<?=$due_amount?>">
                            </div>
                    </div>
                    <div class="control-group">
                        <label class="control-label">পেমেন্ট টাইপ</label>
                    ...
                    <div class="control-group">
                        <div class="controls">
                            <input type="submit" name="update_expense" value="সাবমিট ">
                        </div>
                    </div>
            ...
            elseif($expense == "expense_details")  {
          $expense_id = $_GET['expense_id'];
           ...
           ?>
            ...
            <div class="">
                 ...
                 <?php  
                 $list_expense=mysqli_query($conn,"SELECT * FROM manage_expense JOIN account_head on manage_expense.account_head_id=account_head.account_head_id WHERE expense_id = '$expense_id'");  //1 = Show Data
                 if($list_expense==true)
                 {
                    while($data=mysqli_fetch_array($list_expense))
                    { 
                        ...
                        <div class="row-fluid invoice-list">
                                 <div class="span12">
                                     <h4>INVOICE INFO</h4>
                                     <ul class="unstyled">
                                         <li>সিরিয়াল	: <h4 class="alert-heading"><?=$expense_serial?></h4></li>
                                         <li>তারিখ	: <?=$bd_time?></li>
                                     </ul>
                                 </div>
                             </div>
                             ...
                             <div class="row-fluid">
                                 <table class="table table-striped table-hover">
                                     ...
                                     <tbody>
                                     <tr>
                                         <td class="hidden-480"><?php echo $expense_serial?></td>
                                         <td class="hidden-480"><?php echo $account_head_name?></td>
                                         <td class="hidden-480"><?php echo $expense_title?></td>
...
<?php }?>
	...
	<?php	} ?>  <!-- Ending of ISSET add expense -->
