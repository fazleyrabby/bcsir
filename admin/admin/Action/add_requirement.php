...
{   
	...
	$requrmnt_descrb=mysqli_real_escape_string($_POST['requrmnt_descrb']);
	...
	//echo mysqli_error($conn);
		...
		$add_requrmnt_form=mysqli_query($conn,"INSERT into user_requirement values('','$usernamee','$requrmnt_name','$requrmnt_descrb','$requrmnt_price',1,now(),now(),$sender_id)"); 
		...
		if($res==TRUE)
			{
				...
				$req_details="INSERT into requirement_detalis_tbl values('',$mid,$sender_id,$reciever,$sender_id,'',now(),now(),1)";
...
$req_id=$_POST['req_id'];
		...
		$edit_requrmnt_form=mysqli_query($conn,"UPDATE user_requirement set Qu_title='$requrmnt_name',Qu_descrbs='$requrmnt_descrb',Qu_price='$requrmnt_price' where user_requr_id=$req_id"); 
		//$res=("$conn,$add_requrmnt_form");
		//1 to show data
...
// echo "$user_rcvr_id";
// die();
	$req_details="INSERT into requirement_detalis_tbl values('',$user_requr_id,$sender,$user_rcvr_id,$sender,'$requrmnt_comments',now(),now(),$req_status)";
