     ...
     <div class="span12">
		...
		if (isset($_GET['requrmnt'])) {    
           ...
           <!-- UFR END -->
			...
			elseif ($requrmnt_form == "reg_requrmnt_list") { ?> <!--  if clicked requeriment list   -->
            ...
            <table class="table table-striped table-bordered" id="sample_1">
             ...
             <?php  
               ...
               // ====REJECTED LIST FROM MD  TO ACCOUNTS(RMA)=====
             if($level==2){?>
               ...
               <?php  
                    ...
                    $md_rejc_list= mysqli_query($conn,"SELECT * 
                    FROM user_requirement
                    LEFT JOIN 
                    requirement_detalis_tbl ON requirement_detalis_tbl.id=user_requirement.user_requr_id where user_requirement.Qu_status = 6");
               //1 = Show Data
               echo"<span><h3> এমডি রিজেক্টেড-তালিকা</h3></span>";
                    if($md_rejc_list==true)
                    {
                        while($row=mysqli_fetch_array($md_rejc_list))
                            { 
                                $reqrmnt_id=$row['id'];
                            ...
                            <td>$recv_con_type</td>
                            <td>$confirm_id</td>
                            <td><a href='Action/add_requirement.php?approve_form=approve_requermnt&user_requr_id=$user_requr_id&status=7'><span class='btn btn-success'>Send</span></a>
                            &nbsp;&nbsp;&nbsp;&nbsp;
                            
                            <td><a href='Action/add_requirement.php?approve_form=approve_requermnt&user_requr_id=$user_requr_id&status='><span class='btn btn-success'>Cancel</span></a>
                            &nbsp;&nbsp;&nbsp;&nbsp;
                            </td>";
                            echo "</tr>";
                        }
                        ...
                        // ====REJECTED LIST FROM ACCOUNTS TO USER(RAU)=====

             elseif($level==4){  

                    $rjec_acc_list=mysqli_query($conn,"SELECT * 
                    FROM user_requirement
                    LEFT JOIN 
                    requirement_detalis_tbl ON requirement_detalis_tbl.id=user_requirement.user_requr_id where user_requirement.Qu_status =7");
   
                 echo"<span><h3>একাউন্টস রিজেক্টেড-তালিকা</h3></span";
                    if($rjec_acc_list==true)
                    {
                            while($row=mysqli_fetch_array($rjec_acc_list))
                            { 
                                $reqrmnt_id=$row['id'];
                                $Qu_title= $row['Qu_title'];
                                $sender_id=$row['Qu_price'];
                        ...
                        <td>$sender_id</td>
                        <td> $recv_con_type</td>
                        <td><a href='Action/add_requirement.php?approve_form=approve_requermnt&user_requr_id=$user_requr_id&status='><span class='btn btn-success'>print</span></a>
                        &nbsp;&nbsp;&nbsp;&nbsp;
                        </td>";
                        echo "</tr>";
                        }
                     ...
                     elseif($level==11){  //APPROVE FROM ACCOUNTS/ PENDING ON MD lvl =3
